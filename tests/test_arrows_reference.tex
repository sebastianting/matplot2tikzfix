\begin{tikzpicture}

\definecolor{darkgray176}{RGB}{176,176,176}
\definecolor{steelblue31119180}{RGB}{31,119,180}

\begin{axis}[
axis equal image,
height=5.6666667in,
tick align=outside,
tick pos=left,
width=5.3333333in,
x grid style={darkgray176},
xmin=0, xmax=8,
xtick style={color=black},
y grid style={darkgray176},
ymin=0, ymax=8.5,
ytick style={color=black}
]
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,7.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,6.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,5.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,4.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,3.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,2.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,1.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:3.2,0.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,7.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,6.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,5.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,4.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,3.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,2.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,1.8) circle [radius=0.2];
\draw[draw=none,fill=steelblue31119180] (axis cs:7.2,0.8) circle [radius=0.2];
\draw[-,draw=black] (axis cs:2,7.8) -- (axis cs:3.2,7.8);
\draw (axis cs:2,7.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{-};
\draw[->,draw=black] (axis cs:2,6.8) -- (axis cs:3.2,6.8);
\draw (axis cs:2,6.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{-$>$};
\draw[-|,draw=black] (axis cs:2,5.8) -- (axis cs:3.2,5.8);
\draw (axis cs:2,5.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{-[};
\draw[-latex,draw=black] (axis cs:2,4.8) -- (axis cs:3.2,4.8);
\draw (axis cs:2,4.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{-$|$$>$};
\draw[<-,draw=black] (axis cs:2,3.8) -- (axis cs:3.2,3.8);
\draw (axis cs:2,3.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$<$-};
\draw[<->,draw=black] (axis cs:2,2.8) -- (axis cs:3.2,2.8);
\draw (axis cs:2,2.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$<$-$>$};
\draw[<-[,draw=black] (axis cs:2,1.8) -- (axis cs:3.2,1.8);
\draw (axis cs:2,1.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$<$-[};
\draw[latex-,draw=black] (axis cs:2,0.8) -- (axis cs:3.2,0.8);
\draw (axis cs:2,0.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$<$$|$-};
\draw[latex-latex,draw=black] (axis cs:6,7.8) -- (axis cs:7.2,7.8);
\draw (axis cs:6,7.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$<$$|$-$|$$>$};
\draw[|-,draw=black] (axis cs:6,6.8) -- (axis cs:7.2,6.8);
\draw (axis cs:6,6.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{]-};
\draw[]->,draw=black] (axis cs:6,5.8) -- (axis cs:7.2,5.8);
\draw (axis cs:6,5.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{]-$>$};
\draw[|-|,draw=black] (axis cs:6,4.8) -- (axis cs:7.2,4.8);
\draw (axis cs:6,4.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{]-[};
\draw[-latex,very thick,draw=black] (axis cs:6,3.8) -- (axis cs:7.2,3.8);
\draw (axis cs:6,3.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{fancy};
\draw[-latex,very thick,draw=black] (axis cs:6,2.8) -- (axis cs:7.2,2.8);
\draw (axis cs:6,2.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{simple};
\draw[-latex,very thick,draw=black] (axis cs:6,1.8) -- (axis cs:7.2,1.8);
\draw (axis cs:6,1.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{wedge};
\draw[|-|,draw=black] (axis cs:6,0.8) -- (axis cs:7.2,0.8);
\draw (axis cs:6,0.8) node[
  scale=0.7,
  fill=white,
  draw=black,
  line width=0.4pt,
  inner sep=3pt,
  anchor=east,
  text=black,
  rotate=0.0
]{$|$-$|$};
\end{axis}

\end{tikzpicture}
